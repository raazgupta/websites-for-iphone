	...
	for wordPos in range(0, wordLen):
		try:
			...
			if sen.parent.name != 'script':
				#only print sen if next element of senList is found in 3 steps. For last sen just print.
				if wordPos == (wordLen - 1):
					printCheck(sen)
				else:
					#check number of steps to next element of senList
					steps = 0
					currentSen = sen
					while currentSen != senList[wordPos+1]:
						steps = steps + 1
